...
if (!fs_1.existsSync(logsDir)) {
    fs_1.mkdirSync(logsDir, { recursive: true });
}
// Custom format that sanitizes sensitive data
const sanitizeFormat = winston_1.format((info) => {
...
const fileFormat = winston_1.format.combine(winston_1.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }), winston_1.format.errors({ stack: true }), sanitizeFormat(), winston_1.format.json());
// Define format for console output
const consoleFormat = winston_1.format.combine(winston_1.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), winston_1.format.colorize({ all: true }), winston_1.format.printf((info) => {
...
// Add file transports
const sessionLogFile = path_1.join(logsDir, getLogFilename());
// Session log file (created on startup)
transports.push(new winston_1.transports.File({
    filename: sessionLogFile,
    format: fileFormat,
}));
// Error log file (rotating)
transports.push(new winston_daily_rotate_file_1({
    filename: path_1.join(logsDir, 'error-%DATE%.log'),
...
transports.push(new winston_daily_rotate_file_1({
    filename: path_1.join(logsDir, 'combined-%DATE%.log'),
    datePattern: 'YYYY-MM-DD',
    maxSize: '50m',
    maxFiles: '7d',
    format: fileFormat,
...
transports.push(new winston_daily_rotate_file_1({
    filename: path_1.join(logsDir, 'http-%DATE%.log'),
    datePattern: 'YYYY-MM-DD',
    level: 'http',
    maxSize: '50m',
    maxFiles: '3d',
...
const logger = winston_1.createLogger({
    ...
    exceptionHandlers: [
        new winston_1.transports.File({
            filename: path_1.join(logsDir, 'exceptions.log'),
            format: fileFormat,
        }),
    ],
    rejectionHandlers: [
        new winston_1.transports.File({
            filename: path_1.join(logsDir, 'rejections.log'),
            format: fileFormat,
        }),
    ],
});
...
function requestLogger(req, res, next) {
    ...
    const requestId = req.headers['x-request-id'] || crypto_1.randomUUID();
